$json = Get-Content -Path "c:\HandsOn\handson-app\pages_data.json" -Raw -Encoding UTF8 | ConvertFrom-Json
$basePath = "c:\HandsOn\handson-app\src\app\(site)"
$utf8NoBom = New-Object System.Text.UTF8Encoding($False)

foreach ($page in $json) {
  $dirPath = Join-Path $basePath $page.slug
...
export const metadata = {
  title: '$($page.title) | Hands On! Modernização',
  description: '$($page.description -replace "<br/><br/>", " ")'
}
  ...
  return (
    <div className="bg-white min-h-screen pt-16">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <p className="text-xl text-slate-600 mb-12 leading-relaxed" dangerouslySetInnerHTML={{ __html: '$($page.description)' }} />
        
        <div className="prose prose-lg text-slate-700 max-w-none">
          <h2 className="text-2xl font-bold text-slate-800 mt-8 mb-4">$($page.h2_1)</h2>
          <p className="mb-8" dangerouslySetInnerHTML={{ __html: '$($page.p_1)' }} />
          
          <h2 className="text-2xl font-bold text-slate-800 mt-8 mb-4">$($page.h2_2)</h2>
          <p className="mb-12" dangerouslySetInnerHTML={{ __html: '$($page.p_2)' }} />
        </div>
      </div>
...
"@
  ...
  Write-Host "Generated $($page.slug)"
}
