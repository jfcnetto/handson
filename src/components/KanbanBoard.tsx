...
export default function KanbanBoard({ initialLeads, pricingTiers = [] }: { initialLeads: any[], pricingTiers?: any[] }) {
  ...
  const premiumEmailBody = selectedLead ? `Olá, equipe da ${selectedLead.company || 'sua empresa'}! Tudo bem?\n\nVi que vocês têm uma ótima reputação no Google, mas identifiquei uma oportunidade de melhorar a presença digital e atrair mais clientes.\n\nPodemos criar sites modernos, rápidos e preparados para celular.\n\nPodemos desenvolver uma demonstração sem compromisso para vocês avaliarem.\n\nPosso enviar?\n\nJoão Neto` : '';
